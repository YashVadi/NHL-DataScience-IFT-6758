...
df2016 = pd.read_csv("data/FE1/20162017_clean.csv")
df2017 = pd.read_csv("data/FE1/20172018_clean.csv")
df2018 = pd.read_csv("data/FE1/20182019_clean.csv")
df2019 = pd.read_csv("data/FE1/20192020_clean.csv")
df = pd.concat([df2016, df2017, df2018, df2019])

cleaned_df =  df[df['distance']!='shoot out']

cleaned_df['distance'] = pd.to_numeric(cleaned_df['distance'], errors='coerce')
cleaned_df['angle'] = pd.to_numeric(cleaned_df['angle'], errors='coerce')
cleaned_df['distance']=cleaned_df.distance.astype('float')
cleaned_df['angle']=cleaned_df.angle.astype('float')
cleaned_df = cleaned_df[(~cleaned_df.distance.isna())]


plot_df = cleaned_df.reset_index(drop=True).copy()
cleaned_df.info()

fig = plt.figure(figsize=(20, 20))
        
ax = sns.jointplot(data=plot_df[['distance', 'angle']], x="distance", y="angle", kind="hist", ratio=4, marginal_kws=dict(bins=50),joint_kws=dict(bins=50))
...
ax.savefig(os.path.join("assets/images/2Dhist_shot_angle_distance.png"))  
plt.show()
