...
def create_json_file (year, type, round, matchup, game):
    ...
    file_path = "./" + str(year) + "/" + str(type) + "/" + game_id + ".json" 
    if not Path(file_path).exists():
        with open(file_path, "w") as outfile:
            json.dump(get_game_data(compose_api_url(game_id)), outfile)
...
def __main__ ():
    for year in range(2016, 2021):
        ...
        for type in ["02", "03"]:
            ...
            if type == "02":
                if year == 2016:
                    Path("./2016/02").mkdir(parents=True, exist_ok=True)
                    for game in range(1, 1231):
                        create_json_file(year, type, None, None, game)
                elif 2017 <= year <= 2019:
                    Path("./" + str(year) + "/02").mkdir(parents=True, exist_ok=True)    
                ...
                Path("./" + str(year) + "/03").mkdir(parents=True, exist_ok=True)
                for round in range(1, 5):
                    if round == 1:
                        for matchup in range(1,8):
